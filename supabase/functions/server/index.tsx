...
async function getUserFromToken(accessToken, impersonationToken = null) {
  ...
  if (tokenToCheck?.startsWith('imp_')) {
    const impersonationData = await kv.get(`impersonation:${tokenToCheck}`);
    if (!impersonationData) {
      return { user: null, error: 'Invalid impersonation token' };
    }

    // Check if token expired
    if (new Date(impersonationData.expiresAt) < new Date()) {
      await kv.del(`impersonation:${tokenToCheck}`);
      return { user: null, error: 'Impersonation token expired' };
    }

    // Return user object with ID
    return {
      user: { id: impersonationData.userId },
      error: null
    };
  }
...
app.post("/make-server-8dc4138c/manager/login-as/:userId", async (c) => {
  try {
    ...
    // Create a temporary impersonation token
    const impersonationToken = `imp_${Date.now()}_${Math.random().toString(36).substring(7)}`;

    // Store impersonation session with user ID
    await kv.set(`impersonation:${impersonationToken}`, {
      userId: userId,
    ...
    console.log('Created impersonation token for user:', userId, 'with affiliate ID:', kvUserData.affiliateId);
    return c.json({
      success: true,
